...
to ``isomorphic'' or even to natural isomorphisms in the Yoneda lemma.
...
\end{CD}
\end{equation}
there is a morphism $h = g \circ f : A \to C$.\footnote{That is,
...
statistical models & functions measurable in $x$ and continuous in $\theta$
\end{tabular}
\end{center}
...
$P(x)$ is not inhabited, there is no pair ether.
...
indexed family of sets then $\prod_{i \in I} \mathcal{A}$ is the set of
functions $f : I \to \bigcup \mathcal{A}$
...
is guaranteed by the pairing axiom of ZFC.  Then HoTT defines terms of
...
For example, $1$ is a mere proposition but $1 + 1$ has two elements that
are not equal.  So it isn't.  So if we want to get what people usually
...
This is Definition~3.7.1 {in} \citetalias{hott-book}.
...
This is similar to the discussion of the subset relation in the preceding
section.  A term of $\sum_{x : A} P(x)$ is an ordered pair $(x, p)$ where
$x : A$ and $p$ is a proof of the proposition $P(x)$.  A term of its
propositional truncation forgets both $x$ and $p$ (more precisely, it forgets
that there are any differences between this $x$ and $p$ and any other
$y : A$ and $q : P(y)$).
...
truncation.  But (again perhaps suprisingly) the HoTT book does not.
...
We are used to inverses being unique.
They are in category theory.\footnote{First we prove that identity functions
are unique.  If $\idfun_A$ and $\idfun_A'$ are two identity functions $A \to A$
then $\idfun_A = \idfun_A \idfun_A' = \idfun_A'$ by the identity laws.
Then suppose $f$ has a left inverse $h$ and a right inverse $g$, as in
...
write $A \simeq B$ when $f : A \to B$ and $\binv(f)$.
...
because it makes our theorems not applicable to large parts of contempory
mathematics.
