...
are composable.\label{foot:arrows-only}}
...
Consider abstract group theory, where the only sets of interest are groups
and the only functions of interest are group homomorphisms.
The first isomorphism theorem (of group theory,
\citealp[Theorem~II.8.1]{aluffi};
almost every algebraic theory has a similar theorem, even set theory,
\citealp[Theorem~I.2.7]{aluffi})
...
more on this in what follows, Sections~\ref{sec:constructive}
and~\ref{sec:identity} below.}
where the downward arrow denotes the quotient map,\footnote{The
quotient map can be considered the map $x \mapsto x$ with equality
defined differently of the domain and codomain (as explained in the text).}
the lower horizontal arrow denotes the canonical group
isomorphism,\footnote{The isomorphism can be though of as just $f$,
except with equality defined differently on its domain
(as explained in the text).}
and the upward arrow denotes the canonical injection.\footnote{The
canonical injection is just $x \mapsto x$.}
...
A group $G$ is \emph{simple} if for any homorphism $f : G \to H$
...
takes a large book
\citep[more than 300 pages if frontmatter is included]{wilson}.
...
isomorphism that we learn something (get a non-trivial factorization).
Then $\ker(f)$ and $G / \ker(f)$ are not isomorphic to $G$, and we have
factored $G$.
...
A category (like ETCS) has two sorts of variables,
...
This uses uniqueness of inverses
...
In point set topology, the fundamental object is a topological space, which
...
of open sets are open.  Similarly, probability theory a fundamental object
...
satisfying another of Kolmogorov's axioms.}
...
but which has some of that but also a lot of algebra so these books are
also full of commutative diagrams and diagram-chasing proofs.
Moreover, they are full of definitions and concepts that are categorically
...
A joke because it mystifys more than explains.  But monads are a big deal
in functional programming.}

As far as I know category theory has only filtered down to advanced
undergraduate mathematics (courses mostly for math majors).  It is not
...
as in Appendix~A of \citetalias{hott-book} and throughout \citet{rijke}.}
...
it is more philosophically correct to say ``token'' rather than ``term'' here.}
...
proposition $A$ is the same as constructing a term of type $A$.  So we also
...
given them by the rules of the type and no other properties.
...
In HoTT it makes no sense to ask whether terms of different types are
...
$A \to (B \to C)$, the default being $\to$ is right associative.  In more
detail $f : A \to (B \to C)$ is a function that maps $a : A$ to a term of
type $B \to C$, that is, $f(a) : B \to C$ is a function that maps $B \to C$
so we can evaluate it at any point $b : B$ giving $f(a)(b) : C$.}
...
or paths between paths between paths) and so forth ad infinitum
...
product of any family of sets.  One way to state the axiom of choice is
...
depend on $a : A$, the same as the product type $A \times B$.  So there is
...
choice in order for a dependent product type to be inhabited.
It is just true.\footnote{We still need to assume $B(x)$ is inhabited for
...
(Section~\ref{sec:iso} above, except
...
and they are in set theory \citepalias[Section~4.6]{hott-book}.
For any function $f$, $\binv(f)$ is a mere proposition
\citepalias[Theorem~4.3.2]{hott-book} and this is the definition that is the
...
Then we can
consider a dependent type as a function $B : A \to \universe$.
...
$$
which says $\text{idtoequiv}$ has a homotopy inverse
$$
   \text{ua} : (A \simeq B) \to (A =_\universe B)
$$
($\text{ua}$ for univalence axiom).  And this function can be used to
transfer properties from $A$ to $B$, like any other function.
It says that whatever you have proved about $A$ is also proved about $B$.
...
of skeletality condition) but rather that equality is being expanded to
...
The reverse implication, that this function (implication)
is a homotopy equivalence, is not provable in MLTT.

This reverse implication (that $f(x) = g(x)$ for all $x$ implies $f = g$)
...
But then we run out of names.  A 2-type is what (if you have to make up
a name) a 2-groupoid, (having nontrivial paths between paths).
A 3-type is what (if you have to make up
a name) a 3-groupoid, (having nontrivial paths between paths between paths).
And so on.

MLTT cannot prove that any homotopy $n$-levels above sets exist.
HoTT can prove this using univalence \citepalias[Example~3.1.9]{hott-book}.
Not every type is an $n$-type for some $n$.
An example is the two-sphere $S^2$ (like the surface of a globe).
Such a type is then called an $\infty$-groupoid in (higher) category theory.
The HoTT book does not actually prove this, but does construct
\citepalias[Example~8.8.6]{hott-book} a type that is not a homotopy $n$-type
for any $n$.
...
for any $n$, this gives $\proptrunc{A}_n$ inhabiting homotypy level $n$.
...
\begin{align*}
   ...
   \successor & : \nats \to \nats
\end{align*}
The first subscripts the term $0_\nats$ to distingush it from the empty
...
which we can also
write as a function of four arguments $\inductor_\nats(C, c_0, c_s, n)$
and which maps to $C(n)$.
...
\end{align*}
Then we have $\prod_{n : \nats} C(n)$, that is, the function $n \mapsto C(n)$
...
Peano's axioms for arithmetic except for here $C : \nats \to \universe$
...
(the \citetalias{hott-book} says ``most subtile'').
Read slopply, path induction says that if you prove a property for
...
The subtile mistake \citepalias[Remark 1.12.1]{hott-book} is that
the input is the dependent family of types $(x, y) \mapsto (x =_A y)$,
so $x$ and $y$ must vary freely over $A$ for $\inductor_{=_A}$ to apply.
If $x$ is fixed (not varying) then checking something about $\refl_x$
proves nothing.
...
\end{equation}
\end{subequations}
(a generalization of this result appears in Section~\ref{sec:lifting} below).
...
$f$ takes arguments $a : A$.  And the path is not made out of points of $A$.
Its endpoints are points of $A$, but that is all.\footnote{But if a path were
(as it was before HoTT) a function $p : I \to A$, where $I$ is the unit interval
of the real line, then this lifted path would just be $f \circ p$.}
...
satistfied with \citeauthor{martin-lof-mltt}'s justification and provide
...
pre-mathematical, hence fundamental.}
...
(identities).   We have already met one of these, propositional truncation,
...
In order to prove this function is an isomorphism,
...
This involves what seems to be a digression, but isn't (following section).

\subsection{Path Lifting}
\label{sec:lifting}

Here we want to reconsider dependent sum types $\sum_{x : A} B(x)$.
We have already seen that propositions-as-types says we read this as
logical exists (when each $B(x)$ is a mere proposition).
And we have already seen that sets-as-types says we read this as
subset (when each $B(x)$ is a mere set).
Table~\ref{tab:foo} says that in general, we are supposed to read this
as ``total space.''  So what is that?

In algebraic topology, if we have a morphism $f : X \to Y$, where morphism
means \emph{continuous function} in the category of topological spaces,
and if $f$ is \emph{surjective}, which means for every $y : Y$ there exists
$x : X$ such that $y = f(x)$, then we can think of this in two ways.

The first way is the obvious one, this is just like any other function.
The continuity part is, for example, $x_n \to x$ implies $f(x_n) \to f(x)$.

The second way is the backwards one.  Define for $y : Y$,
the \emph{fiber} of $f$ over $y$ to be
$$
   f^{-1}(y) = \set{ x : X : y = f(x) }
$$
This notation $f^{-1}$ conflicts with the other defined
in Section~\ref{sec:iso} above.  Sometimes I say that
the Section~\ref{sec:iso} concept is the point-to-point inverse,
which exists if and only if the function is an isomorphism,
and this concept is the point-to-set inverse
(or point-to-fiber inverse), which exists for
any function.\footnote{There is a also a set-to-set inverse
$$
   f^{-1}(B) = \set{ x \in X : f(x) \in B }
$$
where $B \subset Y$, which we can translate into HoTT as
$$
   f^{-1}\left(\prod_{y : Y} B(y)\right) :\equiv
   \prod_{x : X} B\bigl(f(x)\bigr)
$$
where $B : Y \to \universe$.
\label{foot:set-to-set}
}
But real mathematicians happily deal with the ambiguity of notation
(although occasionally apologizing for ``abuse of notation'') and don't
say any such thing about the notation.

Now the surjectivity of $f$ says every fiber $f^{-1}(y)$ is inhabited,
and the continuity of $f$ says the fibers are pasted together continuously
to make $X$.\footnote{More precisely, the definition of continuous function
in general topology (also called point set topology) is $f^{-1}(B)$ is an
open subset of $X$ whenever $B$ is an open subset of $Y$, where $f^{-1}$
is the set-to-set inverse of footnote~\ref{foot:set-to-set}.}

Now for the HoTT stuff.  Table~\ref{tab:foo} also says the homotopy
interpretation of a general dependent type family $B : A \to \universe$
is a ``fibration.''  And that means analogous to what this section is
calling the backwards view of a continuous function as
a map $y \mapsto f^{-1}(y)$.  So we have a space (type) $Y$, and a total
space $X \equiv \prod_{y : Y} B(y)$ where $B(y)$ is now interpreted as the
fiber over $y : Y$ and the fibers are pasted together continuously to make $X$,
although in HoTT the ``continuously'' is a mere analogy --- synthetic rather
than analytic --- it doesn't mean anything other than what
the rules of HoTT for the types involved make it mean.

So now we are ready for Lemma~2.3.1 {in} \citetalias{hott-book}.
For any function (dependent type family) $F : A \to B$,
any $x, y : A$ and any $p : x =_A y$, there is a function
$$
   p_{\textstyle *} : F(x) \to F(y)
$$
