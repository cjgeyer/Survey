...
\title{Homotopy Type Theory Basics}
...
This document goes over some basic principles of homotopy type theory (HoTT).
It follows \citet[hereinafter referred to as the HoTT book]{hott-book}.
\defcitealias{hott-book}{the HoTT book}

\subsection{Terms and Types}

Terms (references to mathematical objects) have types.  This is symbolized
$a : A$, read $a$ is a term of type $A$.
Every term has exactly one type.  It makes no sense for a term to
be untyped or to have more than one type.  We say the mathematics or computer
program \emph{type checks} if we can verify that each term has its asserted
type.

Unlike logic plus set theory

Types are defined
\emph{intensionally} by rules for constructing and using terms of the type.

There are alternative readings of $a : A$
\begin{itemize}
\item $a$ is a proof of the proposition $A$,
\item $a$ is an element of the set $A$, and
\item $a$ is a point in the space $A$.
\end{itemize}

The first alternative reading is called \emph{propositions as types}.
This interpretation says $A$ is true if it is inhabited and false otherwise.
Thus $a : A$ says that $A$ is true.  It proves $A$ is true.

The first alternative reading is called \emph{propositions as types}.
\REVISED
We can instead read $a : A$ as $a$ is an element of the set $A$.
But types are not sets like in ZFC (Zermelo–Fraenkel set theory with
the axiom of choice) or similar set theories. 

ZFC-like sets are defined \emph{extensionally:} they are nothing but
a collection of somethings.  In type theory, terms and types are different
kinds of things.  In ZCF-like set theory, elements and sets are the same
kind of thing: elements of sets are themselves sets.  In type theory,
every term has exactly one type.  In ZFC-like set theory, elements can
belong to more than one set.  So this reading can be misleading and we
have to keep the differences in mind.

We can say instead $a$ is a point in a space $A$.  This says $A$ has structure
over and above being a mere set.  It does not behave as just a collection.

We can say instead $a$ is a proof of the proposition $A$.
Martin-L\"{o}f type theory (MLTT) \citep{martin-lof-mltt}
and HoTT have the notion of
\emph{propositions as types}.  A type considered as a proposition is true
if inhabited and false if uninhabited.  Thus an inhabitant proves
the proposition, that is, $a : A$ is a proof of $A$.
There is no difference between proving a proposition and constructing a
term of the type.

But there is only one concept type, not three concepts (proposition, set,
space).  We may consider this concept in different ways depending on what
we are doing, but there is no difference.

\subsection{Operations on Types}

We can combine types and terms via various operations
...
$A + B$ & sum or coproduct type \\
$\prod_{x : A} B(x)$ & dependent function type \\
$\sum_{x : A} B(x)$ & dependent pair or sum type \\
$x =_A y$ & identity type
\end{tabular}
\end{center}
\caption{Operations on Types (Capital Letters) and Terms (Lower Case Letters).}
\label{tab:operations}
\end{table}

Most of these are familiar to mathematicians.  A term of type $A \to B$
is a function $f : A \to B$ that maps $x : A$ to $f(x) : B$.  The rules
for the type say how to construct $f(x)$ from $x$.

A term of type $A \times B$ (the Cartesian product of $A$ and $B$) is an
ordered pair $(a, b)$, where $a : A$ and $b : B$.

Type $A + B$ is less familiar.  It can be thought of as the disjoint
union in set theory.  Formally, a term of type $A + B$ has the form
$\text{inl(a)}$ for some $a : A$ or
$\text{inr(b)}$ for some $b : B$, that is, either a term of type $A$ or a
term of type $B$ together with the memory of which.

A term of type $\prod_{x : A} B(x)$ (the Cartesian product of arbitrary family)
is again familiar.  It is a function $f : \prod_{x : A} B(x)$ that maps $x : A$
to $f(x) : B(x)$.  The only difference with an ordinary function is that
the codomain $B(x)$ depends on the argument $x$.
But this is just like the way a Cartesian product of an arbitrary family of
sets is defined in ZFC-like set theory.

A term of type $\sum_{x : A} B(x)$ is an ordered pair $(x, y)$ where $x : A$
and $y : B(x)$.  So this is just like a term of type $A \times B$ except that
the type of $y$ depends on $x$.

\subsection{Propositions As Types}

\emph{Propositions as types} is the view of types as propositions.
A type considered as a proposition is true if inhabited and false
if uninhabited.  A term $a : A$ is a proof that $A$ is true.
So types are propositions and terms are proofs.
All of the operations of type theory have reinterpretations in
the propositions-as-types view
\begin{table}
\begin{center}
\begin{tabular}{lll}
operation & logical interpretation & logical notation \\
\hline
$0$ & empty type, falsity & $\bot$ \\
$1$ & unit type, truth & $\top$ \\
$A \to B$ & logical implication & $A \Rightarrow B$ \\
$A \times B$ & logical and (conjunction) & $A \vee B$ \\
$A + B$ & logical or (disjunction) & $A \wedge B$ \\
$\prod_{x : A} B(x)$ & logical for all & $\forall_{x : A} B(x)$ \\
$\sum_{x : A} B(x)$ & logical exists & $\exists_{x : A} B(x)$ \\
$x =_A y$ & logical equality & $x = y$ \\
$A \to 0$ & logical negation & $\neg A$
\end{tabular}
\end{center}
\caption{Propositions as Types.}
\label{tab:propositions-as-types}
\end{table}

\subsection{The Empty Type}

We start with the empty type $0$.  This type is very different from the
rest.  It has no constructors that say how to make terms of the type.
We can think of it as the type having no terms, but to formalize that we
need elimination rules \citepalias[Section~1.7]{hott-book}.
The recursor for the empty type says for any type $A$ there is a function
$0 \to A$.

This is getting a bit ahead of ourselves because we have not defined function
types yet.  But we can say that HoTT functions are just like the rest of math
in that for $f : A \to B$ we have $f(x) : B$ for each $x : A$.

The inhabitant of $0 \to A$ is the empty function that has a vacuous rule.
We do have $f(x) : A$ for all $x : 0$ but there are no such $x$.

In the propositions-as-types view we interpret 0 as falsity.  And we
interpret $0 \to A$ as the logical principle \emph{ex falso quodlibet}.
...
goes against human languages.  When there are two sorts of variables,
...
\citep[of group theory,][Theorem~II.8.1]{aluffi}\footnote{Almost every
algebraic theory
has a similar theorem, even set theory \citep[Theorem~I.2.7]{aluffi}.}
...
maps $S \to \Omega$, where for classical logic $\Omega$ is isomorphic to
...
witness or evidence instead of proof.}
\label{tab:term-type}
\end{table}
...
a dependent function type $f : \prod_{x : A} B(x)$ is just a function such
that $f(x) : B(x)$ so the codomain of the function depends on the argument $x$.
In set theory, of course, we have exactly the same thing,
...
defines two other types that turn out to be
equivalent to bi-equivalence \eqref{eq:bi-invertible-type}.
Any could be used to define \emph{homotopy equivalence}.
...
have to do to prove $P : \nats \to \universe$ is to verify $P(0)$ and
...
The subtle mistake \citepalias[Remark 1.12.1]{hott-book} is that
...
more careful, and this is exactly the same care that is required for
...
Another deals with \emph{strictly monotone} functions $f$, those for which
$x < y$ implies $f(x) < f(y)$.  Then \citep[Theorem~5.5]{bauer-constructive},
if $x < y$ and $f$ is a strictly monotone function such that $f(x) < 0 < f(y)$,
there there exists a unique $z$ such that $x < z < y$ such that $f(z) = 0$.
...
\item \textbf{paths:} If $u, v : \real_c$
    such that $\prod_{\varepsilon : \rats_+} (x \sim_\varepsilon y)$,
...
\begin{multline*}
   ...
   + \bigl( m(i(x), x) = e \bigr) \right)
   \\
   \times
   \left( \prod_{x : G} \bigl( m(x, e) = x \bigr)
   + \bigl( m(e, x) = x \bigr) \right)
\end{multline*}
...
the last axiom for $\sim$ imposes the UIP property on $\sim_\varepsilon$.

So HoTT does not make classically trained mathematicians do anything they
don't want to.  It just makes them be explict about it.
...
combined with addresses (numbers of memory locations) made machine-language
...
not allowed to refer to anything but rather to objects of some specific type.
...
have had a year of Euclidean geometry in high school.  Also we read long
ago in FIG \citet{bell}, a book about synthetic differential geometry.
